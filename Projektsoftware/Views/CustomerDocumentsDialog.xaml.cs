...
{
    public partial class CustomerDocumentsDialog : Window
    {
        private readonly Customer customer;
        private readonly DatabaseService databaseService;
        private readonly ObservableCollection<CustomerDocument> documents;
        private readonly string documentsBasePath;

        public CustomerDocumentsDialog(Customer customer)
        {
            InitializeComponent();

            this.customer = customer;
            databaseService = new DatabaseService();
            documents = new ObservableCollection<CustomerDocument>();
            DocumentsDataGrid.ItemsSource = documents;

            documentsBasePath = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                "Projektsoftware", "Documents", "Customers", customer.Id.ToString());

            CustomerNameTextBlock.Text = $"Kunde: {customer.DisplayName}";
        ...
        private async System.Threading.Tasks.Task LoadDocumentsAsync()
        {
            try
            {
                ...
                var loadedDocs = await databaseService.GetCustomerDocumentsAsync(customer.Id);
        ...
        private async void DeleteDocument_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                await databaseService.DeleteCustomerDocumentAsync(doc.Id);
